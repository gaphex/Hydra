...
def fetch_from_mail():
    ...
    save_attachment(m, items[-1])

    fn = [t for t in os.listdir(detach_dir) if t.startswith('proxylist')][-1]
    with zipfile.ZipFile(detach_dir + fn, "r") as z:
        z.extractall(detach_dir)

    f = open(detach_dir + 'full_list/_reliable_list.txt', 'r')
